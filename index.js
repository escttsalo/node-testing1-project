...
function findLargestInteger(integers) {
  let res = integers[0].integer
  for (let i=1; i < integers.length; i++){
    if (integers[i].integer > res){
      res = integers[i].integer
    }
  }
  return res
}
...
class Seasons {
   ...
   */
  next() {
    const res = this.seasons[this.currentSeason]
...
class Car {
   ...
   */
  drive(distance) {
    const milesDrivable = this.tank * this.mpg
    if (distance <= milesDrivable){
      this.odometer += distance
      this.tank = this.tank - (distance/this.mpg)
    } else {
      this.odometer = this.odometer + milesDrivable
      this.tank = 0
    }
